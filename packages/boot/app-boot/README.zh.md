...
- **进程内模块解析。** launcher 在挂载 profile 条目前，将 runtime resolution 安装到 Node 的 ESM 与 CommonJS 内部 resolver。exports、conditions、子路径、模块缓存和错误码仍由 Node 负责；路由后的 ESM 失败报告原始 importer，该诊断的改写为尽力而为，因此 resolver 错误上的 `stack` 只读时仍保留其错误码。显式 CommonJS `paths` 始终保留原生查询，包括指向 profile 内的路径。
