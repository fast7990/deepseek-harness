...
- **Process-local module resolution.** The launcher installs the runtime resolution on Node's internal ESM and CommonJS resolvers before profile rows mount. Node still owns exports, conditions, subpaths, module caches, and error codes; routed ESM failures report the original importer, and rewriting that diagnostic is best-effort, so a host whose resolver errors expose a read-only `stack` still receives the resolver's code. Explicit CommonJS `paths` always retain native lookup, including paths inside profiles.
